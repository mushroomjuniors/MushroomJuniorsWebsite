...
export default function FranchisePage() {
  ...
  const onSubmit = async (data: InquiryFormValues) => {
    try {
      ...
      if (result.isSuccess) {
        toast.success("Franchise Inquiry Sent!", {
          description: result.message || "Thank you for your interest in our franchise. We'll get back to you soon.",
        })
        reset()
      } else {
        if (result.fields) {
    ...
    <div className="flex flex-col min-h-screen">
      <div className="relative">
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('/franchise-bg.avif')`,
          }}
        >
          <div className="container px-4 mx-auto text-center z-10">
            <h1 className="text-4xl font-bold tracking-tight text-white sm:text-5xl md:text-6xl">Franchise With Us</h1>
            <p className="mt-4 text-xl text-white max-w-3xl mx-auto">
              Interested in joining our growing family? Fill out the form below to inquire about franchise opportunities.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="container px-4 py-16 mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2">
            <h2 className="text-3xl font-bold mb-6">Franchise Inquiry Form</h2>
            {isSubmitSuccessful && !isSubmitting ? (
              <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
                ...
                <p className="text-muted-foreground mb-4">
                  Thank you for your interest in our franchise. We'll get back to you as soon as possible.
                </p>
                <Button onClick={() => reset()}>Send Another Inquiry</Button>
              </div>
            ) : (
              <Form {...form}>
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  <FormField control={control} name="subject" render={({ field }) => (
                    <FormItem><FormLabel>Subject</FormLabel><FormControl><Input {...field} disabled /></FormControl><FormMessage /></FormItem>
                  )} />
                  <FormField control={control} name="message" render={({ field }) => (
                    <FormItem><FormLabel>Tell us about yourself and your interest in our franchise</FormLabel><FormControl><Textarea rows={5} {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
          ...
          <div className="space-y-8">
            <div>
              <h3 className="text-xl font-bold mb-4">Franchise Contact</h3>
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  <Store className="h-5 w-5 text-red-600 mt-0.5" />
                  <div>
                    <h4 className="font-medium">Franchise Department</h4>
            ...
            <div>
              <h3 className="text-xl font-bold mb-4">Why Franchise With Us?</h3>
              <div className="space-y-4">
                <div>
                  <h4 className="font-medium">Proven Business Model</h4>
                  <p className="text-muted-foreground">
                    Join a brand with a track record of success and a loyal customer base.
                  </p>
                </div>
                <div>
                ...
                <div>
                  <h4 className="font-medium">Growing Market</h4>
                  <p className="text-muted-foreground">
                    Be part of a rapidly expanding industry with high demand.
                  </p>
                </div>
              </div>
            </div>
