...
export function ProductForm({ initialData, categories }: ProductFormProps) {
  ...
  async function onSubmit(data: ProductFormValues) {
    ...
    let existingGalleryImageUrls = initialData?.image_urls || [];
    ...
    try {
      ...
      let finalGallerySet = processedPrimaryImageUrl ? [processedPrimaryImageUrl] : [];
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <FormField control={form.control} name="name" render={({ field }) => (
            <FormItem><FormLabel>Product Name</FormLabel><FormControl><Input placeholder="e.g., Organic Cotton T-Shirt" {...field} /></FormControl><FormMessage /></FormItem>
        )} />
        ...
        <FormField control={form.control} name="description" render={({ field }) => (
            <FormItem><FormLabel>Description (Optional)</FormLabel><FormControl><Textarea placeholder="Detailed description of the product..." className="resize-none" {...field} value={field.value || ""} /></FormControl><FormMessage /></FormItem>
        )} />

        {/* Price and Stock Quantity Grid */}
        <div className="grid grid-cols-1 gap-y-8 md:grid-cols-2 md:gap-x-6">
            <FormField control={form.control} name="price" render={({ field }) => (
                <FormItem><FormLabel>Price</FormLabel><FormControl><Input type="number" step="0.01" placeholder="e.g., 25.99" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
        ...
        <FormItem>
          ...
          {initialData?.image_url && !primaryImageFile && (
            <div className="mt-2">
              ...
              <img src={initialData.image_url} alt="Current primary" className="h-24 w-24 object-cover rounded-md border" />
            </div>
          )}
          {primaryImageFile && (
             <div className="mt-2">
              <p className="text-sm text-muted-foreground">New primary image preview:</p>
              <img src={URL.createObjectURL(primaryImageFile)} alt="New primary preview" className="h-24 w-24 object-cover rounded-md border" />
            </div>
          )}
        </FormItem>
        ...
        <FormItem>
          ...
          {initialData?.image_urls && initialData.image_urls.filter(url => url !== initialData.image_url).length > 0 && (
            <div className="mt-2">
                <p className="text-sm text-muted-foreground w-full mb-1">Current additional gallery images:</p>
                <div className="flex flex-wrap gap-2">
                {initialData.image_urls.filter(url => url !== initialData.image_url).map((url, index) => (
                    <img key={index} src={url} alt={`Current gallery ${index + 1}`} className="h-20 w-20 object-cover rounded-md border" />
                ))}
                </div>
            </div>
          )}
            ...
            <div className="mt-2">
              ...
              <div className="flex flex-wrap gap-2">
                {Array.from(galleryImageFiles).map((file, index) => (
                  <img key={index} src={URL.createObjectURL(file)} alt={`New gallery preview ${index + 1}`} className="h-20 w-20 object-cover rounded-md border" />
                ))}
              </div>
            </div>
          )}
